...
```{r}
dfbeta1_3_sscorr_10pecent <- data.frame("Estimateur"= as.numeric(c(EST.comp[,2], EST.cc[,2], EST.saem[,2])), "Méthode" = as.factor( c(rep("complet",nbsim*2), rep("cc",nbsim*2), rep("saem",nbsim*2))), "alea"= as.factor(c(EST.comp[,p+2],EST.cc[,p+2],EST.saem[,p+2]))) # dernière colonne du tableau pour créer l'indicatrice MCAR/MAR
```

boxplots esthétiques comparaison MCAR/MAR pour comp, cc et saem, $10\%$ valeurs manquantes $n=500$
```{r}
box <- ggplot(dfbeta1, aes(x=Méthode,y=Estimateur)) + geom_boxplot(aes(fill=alea))
box <- box + geom_hline(yintercept=0.5, linetype="dashed", color = "purple", size=1)
box <- box + ggtitle(expression(paste("Boxplots de l'estimation de ",beta[1]," avec ",p, " covariables")))
