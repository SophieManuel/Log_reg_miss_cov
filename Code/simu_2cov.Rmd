...
initialisation du jeu de données avec 2 covariables et 20% de valeur manquante
```{r}
...
beta.star <- c(0.5,-1) # coefficients of logistic regression
...
#count
print("coverage")
sum(count.comp)/(length(beta.true)*nbsim)
sum(count.cc)/(length(beta.true)*nbsim)
sum(count.saem)/(length(beta.true)*nbsim)

print("longueur des intervalles")
sum(LENGTH.comp[,1])/nbsim
sum(LENGTH.comp[,2])/nbsim

print("moyenne des longueurs de tous les intervalles")
mean(LENGTH.cc)
mean(LENGTH.comp)
mean(LENGTH.saem)
```
...
for (NB in 1:nbsim){
 ...
 X.obs <- X.complete
  ...
  for(i in c(2)){
   z <- cbind(y,X.complete[,1])%*%matrix(c(1,1),ncol=1) # linear combination #(coefficients choosen with Monte Carlo estimation)
  pr <- 1/(1+exp(-z))         # pass through an inv-logit function
   r <- rbinom(n,1,pr)      # bernoulli response variable
   X.obs[r==0,i]<-NA
  }
...
```{r}
dfbeta1 <- data.frame("Estimateur"= c(EST.comp[,2], EST.cc[,2], EST.saem[,2]), "meth" = as.factor( c(rep("comp",nbsim), rep("cc",nbsim), rep("saem",nbsim))), "alea"= as.factor(rep(EST.comp[,4],3))) # dernière colonne du tableau pour créer l'indicatrice MCAR/MAR
```

boxplots esthétiques comparaison MCAR/MAR pour comp, cc et saem, 10% valeurs manquantes n=500
```{r}
#dfbeta1 <- data.frame("comp"=EST.comp[,2],"cc"=EST.cc[,2],"saem"=EST.saem[,2], "alea"= as.factor(EST.comp[,4])) # dernière colonne du tableau pour créer l'indicatice MCAR/MAR
