...
# 2covariables & 10% manquants

initialisation du jeu de données avec 2 covariables et 10% de valeur manquantes
```{r}
...
beta.star <- c(0.5, -0.3) # coefficients of logistic regression
...
```{r}
dfbeta2_2_sscorr_10percent <- data.frame("Estimateur"= as.numeric(c(EST.comp[,3], EST.cc[,3], EST.saem[,3])), "Méthode" = as.factor( c(rep("complet",nbsim*2), rep("cc",nbsim*2), rep("saem",nbsim*2))), "alea"= as.factor(c(EST.comp[,p+2],EST.cc[,p+2],EST.saem[,p+2]))) # dernière colonne du tableau pour créer l'indicatrice MCAR/MAR
```

boxplots esthétiques comparaison MCAR/MAR pour comp, cc et saem, $10\%$ valeurs manquantes $n=500$
```{r}
box <- ggplot(dfbeta2_2_sscorr_10percent, aes(x=Méthode,y=Estimateur)) + geom_boxplot(aes(fill=alea))
box <- box + geom_hline(yintercept=-0.3, linetype="dashed", color = "purple", size=1)
box <- box + ggtitle(expression(paste("Boxplots de l'estimation de ",beta[2]," avec 30% de valeurs manquantes")))
#box <- box + ggtitle(paste(expression("Boxplots de l'estimation de ",beta[2])," en fonction \ndu type d'aléatoire et des méthodes d'estimation sans corrélation\n  échantillon ",n, "avec",p, "variables et ",p.miss*100,"% de valeurs manquantes" )  )
...
#  2 covariables & 10% manquants & Corrélation

initialisation du jeu de données avec 2 covariables, 10% de valeur manquante et corrélation.
```{r}
#n <- 10000  # number of subjects
n <- 500 # or a smaller number of subjects 
p <- 2     # number of explanatory variables
...
C <- cbind(c(1,0.8),c(0.8,1))  # correlation matrix
...
```{r}
i=2
dfbeta2_2_corr_10percent <- data.frame("Estimateur"= as.numeric(c(EST.comp[,i], EST.cc[,i], EST.saem[,i])), "Méthode" = as.factor( c(rep("complet",nbsim*2), rep("cc",nbsim*2), rep("saem",nbsim*2))), "alea"= as.factor(c(EST.comp[,p+2],EST.cc[,p+2],EST.saem[,p+2]))) # dernière colonne du tableau pour créer l'indicatrice MCAR/MAR
```

boxplots esthétiques comparaison MCAR/MAR pour comp, cc et saem, $10\%$ valeurs manquantes $n=500$
```{r}
box <- ggplot(dfbeta2_2_corr_10percent, aes(x=Méthode,y=Estimateur)) + geom_boxplot(aes(fill=alea))
box <- box + geom_hline(yintercept=0.5, linetype="dashed", color = "purple", size=1)
box <- box + ggtitle(expression(paste("Boxplots de l'estimation de ",beta[1]," avec corrélation")))
#box <- box + ggtitle(paste(expression("Boxplots de l'estimation de ",beta[2])," en fonction \ndu type d'aléatoire et des méthodes d'estimation sans corrélation\n  échantillon ",n, "avec",p, "variables et ",p.miss*100,"% de valeurs manquantes" )  )
box + theme(plot.title = element_text(hjust = 0.5))
```
